	...
	<view class="container">
		...
		<view v-if="!hasLogin || empty===true" class="empty">
			...
			<view v-else class="empty-tips">
				空空如也
				<view class="navigator" @click="navToLogin">去授权></view>
			</view>
		</view>
		<view v-else>
			<!-- 列表 -->
			<view class="cart-list">
				<block v-for="(item, index) in collectList" :key="item.id">
					<view class="cart-item" :class="{'b-b': index!== collectList.length-1}">
						<view class="image-wrapper">
							<image class="img" :src="'http://127.0.0.1:3000/'+item.img" :class="[item.loaded]" lazy-load @load="onImageLoad('collectList', index)"
							 @error="onImageError('collectList', index)"></image>
							<!-- <view class="yticon icon-xuanzhong2 checkbox" :class="{checked: item.checked}" @click="check('item', index)"></view> -->
						</view>
						<view class="item-right">
							<text class="clamp title">{{item.menuName}}</text>
	...
	export default {
		...
		methods: {
			...
			async getMenuCollectList(){
				const result =await  this.$common.$service.$menu.getMenuCollectList();
			...
			updataUserInfo(userInfo) {
				this.$common.$service.$user.updataUserInfo(userInfo).then((res) => {
					if (res && res.code == 200) {
						console.log('更新用户成功')
						this.login(userInfo);
					}
